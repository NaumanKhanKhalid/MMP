    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function filterPOs() {
                ...
                $.ajax({
                    url: '<?php echo e(route('purchase-orders.index')); ?>',
                    type: 'GET',
                    data: formData,
                    beforeSend: function() {
        ...
        function openViewModal(poId) {
            $.ajax({
                url: '<?php echo e(route('purchase-orders.view-modal', ':id')); ?>'.replace(':id', poId),
                method: 'GET',
                success: function(response) {
...
<?php $__env->startSection('content'); ?>
    <div class="container-fluid py-4">
        ...
        <div class="card shadow-sm mb-3">
            <div class="card-body">
                <form method="GET" action="<?php echo e(route('purchase-orders.index')); ?>" id="filterForm">
                    <div class="row g-2">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-1">
                            <div class="d-grid gap-1">
                                <button type="button" class="btn btn-outline-info"
                                    onclick="window.location.href='<?php echo e(route('purchase-orders.index')); ?>'">
                                    Reset
                                </button>
                            </div>
                        </div>
        ...
        <div class="card shadow-sm">
                ...
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-3">
                    <div class="card-title">
                        Purchase Orders<span
                            class="badge bg-light text-default rounded ms-1 fs-12 align-middle"><?php echo e($purchaseOrders->total()); ?></span>
                    </div>
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                ...
                                <li><a class="dropdown-item"
                                        href="<?php echo e(route('purchase-orders.export', ['format' => 'pdf'])); ?>">
                                        <i class="ri-file-pdf-line me-2 text-danger"></i>Export as PDF
                                    </a></li>
                                <li><a class="dropdown-item"
                                        href="<?php echo e(route('purchase-orders.export', ['format' => 'csv'])); ?>">
                                        <i class="ri-file-text-line me-2 text-info"></i>Export as CSV
                                    </a></li>
                                <li><a class="dropdown-item"
                                        href="<?php echo e(route('purchase-orders.export', ['format' => 'excel'])); ?>">
                    ...
                    <table class="table table-striped align-middle table-hover" id="purchaseOrdersTable">
                        ...
                        <tbody class="grn_list_tbody            ">
                            <?php $__empty_1 = true; $__currentLoopData = $purchaseOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $purchaseOrder): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="clickable-row" data-po-id="<?php echo e($purchaseOrder->id); ?>">
                                    <td>
                                        <div class="fw-semibold text-primary"><?php echo e($purchaseOrder->po_number); ?></div>
                                        <?php if($purchaseOrder->notes): ?>
                                            <small class="text-muted"><?php echo e(Str::limit($purchaseOrder->notes, 30)); ?></small>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div class="fw-semibold"><?php echo e($purchaseOrder->supplier->name ?? 'N/A'); ?></div>
                                        <?php if($purchaseOrder->supplier): ?>
                                            <small class="text-muted"><?php echo e($purchaseOrder->supplier->email ?? ''); ?></small>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div><?php echo e($purchaseOrder->order_date->format('d/m/Y')); ?></div>
                                        <small class="text-muted"><?php echo e($purchaseOrder->order_date->format('H:i')); ?></small>
                                    </td>
                                    <td>
                                        <?php if($purchaseOrder->expected_delivery_date): ?>
                                            <div><?php echo e($purchaseOrder->expected_delivery_date->format('d/m/Y')); ?></div>
                                            <?php if($purchaseOrder->expected_delivery_date < now()): ?>
                                    ...
                                    <td>
                                        <div class="fw-semibold"><?php echo e($purchaseOrder->items->count()); ?> items</div>
                                        <small class="text-muted">
                                            Qty: <?php echo e($purchaseOrder->items->sum('quantity')); ?>

                                        </small>
                                    </td>
                                    <td>
                                        <div class="fw-semibold text-success">
                                            R <?php echo e(number_format($purchaseOrder->grand_total, 2)); ?>

                                        </div>
                                        <?php if($purchaseOrder->vat_enabled && $purchaseOrder->vat > 0): ?>
                                            <small class="text-muted">VAT: R
                                                <?php echo e(number_format($purchaseOrder->vat, 2)); ?></small>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <div class="fw-semibold"><?php echo e($purchaseOrder->user->name ?? 'N/A'); ?></div>
                                        <small
                                            class="text-muted"><?php echo e($purchaseOrder->created_at->format('d/m/Y')); ?></small>
                                    </td>
                                    <td class="text-end">
                                        <div class="btn-list">
                                            <button type="button" class="btn btn-sm btn-info-light btn-icon view-po-btn"
                                                data-po-id="<?php echo e($purchaseOrder->id); ?>" title="View">
                                                <i class="ri-eye-line"></i>
                                            </button>
                                            <?php if(in_array($purchaseOrder->status, ['draft', 'approved'])): ?>
                                                <button type="button"
                                                    class="btn btn-sm btn-success-light btn-icon edit-po-btn"
                                                    data-po-id="<?php echo e($purchaseOrder->id); ?>" title="Edit">
                                            ...
                                            <?php if($purchaseOrder->status === 'draft'): ?>
                                                <button type="button"
                                                    ...
                                                    data-po-id="<?php echo e($purchaseOrder->id); ?>" title="Approve">
                                                    <i class="ri-check-line"></i>
                                                </button>
                                            <?php endif; ?>
                                            <?php if(in_array($purchaseOrder->status, ['approved', 'sent', 'partially_received'])): ?>
                                                <button type="button"
                                                    class="btn btn-sm btn-warning-light btn-icon create-grn-btn"
                                                    data-po-id="<?php echo e($purchaseOrder->id); ?>" title="Create GRN">
                                                    <i class="ri-truck-line"></i>
                                                </button>
                                            <?php endif; ?>
                                            <button type="button"
                                                class="btn btn-sm btn-primary-light btn-icon print-po-btn"
                                                data-po-id="<?php echo e($purchaseOrder->id); ?>" title="Print">
                                                <i class="ri-printer-line"></i>
                                            </button>
                                            <?php if(!in_array($purchaseOrder->status, ['completed', 'cancelled', 'closed'])): ?>
                                                <button type="button"
                                                    class="btn btn-sm btn-danger-light btn-icon delete-po-btn"
                                                    data-po-id="<?php echo e($purchaseOrder->id); ?>" title="Delete">
            ...
            <div class="card-footer">
                <div id="paginationContainer">
                    <?php echo $__env->make('purchase_orders.partials.pagination', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(function() {
            ...
            $(document).on('click', '.approve-po-btn', function(e) {
                ...
                $.ajax({
                    url: '<?php echo e(route('purchase-orders.approve', ':id')); ?>'.replace(':id', poId),
                    method: 'POST',
                    headers: {
            ...
            function openCreateGRNModal(poId) {
                ...
                $.ajax({
                    url: '<?php echo e(url('goods-receipts/po')); ?>/' + poId + '/items',
                    method: 'GET',
                    success: function(response) {
                        if (response.success) {
            ...
            function loadCreateModal() {
                ...
                $.get('<?php echo e(route('purchase-orders.create-modal')); ?>')
                    .done(function(data) {
            ...
            function loadEditModal(poId) {
                ...
                $.get('<?php echo e(route('purchase-orders.edit-modal', ':id')); ?>'.replace(':id', poId))
                    .done(function(data) {
                        $('#poModalContent').html(data);
                    })
                    .fail(function() {
            ...
            function printPurchaseOrder(poId) {
                const printUrl = '<?php echo e(route('purchase-orders.print', ':id')); ?>'.replace(':id', poId);
            ...
            function deletePurchaseOrder(poId) {
                $.ajax({
                    url: '<?php echo e(route('purchase-orders.destroy', ':id')); ?>'.replace(':id', poId),
                    type: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
        ...
        function printSelectedPOs() {
            ...
            const printUrl = '<?php echo e(route('purchase-orders.index')); ?>?' + params.toString();
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\MMP\resources\views/purchase_orders/index.blade.php ENDPATH**/ ?>