...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-md-12">
            <div class="card {{ request()->hasAny(['status', 'type', 'refund_method', 'search']) ? 'border-primary' : '' }}">
                <div class="card-header {{ request()->hasAny(['status', 'type', 'refund_method', 'search']) ? 'bg-primary text-white' : 'bg-light' }}">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="mb-0">
                            <i class="bi bi-funnel me-2"></i>Filters
                            @if(request()->hasAny(['status', 'type', 'refund_method', 'search']))
                    ...
                    <div class="row g-3 align-items-end">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select" id="typeFilter">
                                ...
                                <option value="full" {{ request('type') == 'full' ? 'selected' : '' }}>Full Return</option>
                                <option value="partial" {{ request('type') == 'partial' ? 'selected' : '' }}>Partial Return</option>
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select" id="refundFilter">
                                ...
                                <option value="store_credit" {{ request('refund_method') == 'store_credit' ? 'selected' : '' }}>Store Credit</option>
                        ...
                        <div class="col-md-2">
                            ...
                            <input type="text" class="form-control" id="searchInput" placeholder="Search returns..." value="{{ request('search') }}">
                        </div>
                        @if(!request()->hasAny(['status', 'type', 'refund_method', 'search']))
                        <div class="col-md-1">
                    ...
                    <tbody id="returnsTableBody">
                        ...
                        <tr>
                            <td>
                                ...
                                <br>
                                <small class="text-muted d-md-none">{{ $return->created_at->format('M d, Y') }}</small>
                            </td>
                            <td>
                            ...
                            <td class="d-none d-lg-table-cell">
                                <strong>R {{ number_format($return->total_amount, 2) }}</strong>
                            </td>
                                ...
                                <div class="btn-list">
                                    ...
                                    @if($return->status === 'pending')
                                        ...
                                        <button type="button" class="btn btn-sm btn-warning-light btn-icon" 
                                                onclick="editReturn({{ $return->id }})" title="Edit">
                                    ...
                                    @if($return->status === 'approved')
                                        ...
                                        <button type="button" class="btn btn-sm btn-primary" 
                                                onclick="completeReturn({{ $return->id }})" title="Complete Return">
                                            <i class="bi bi-check-double me-1"></i>Complete
                                        </button>
                                    @endif
                                    
                                    @if($return->status === 'completed' && $return->creditNote->count() > 0)
                                        <!-- Download Credit Note -->
                                        <button type="button" class="btn btn-sm btn-secondary-light btn-icon" 
                                                onclick="downloadCreditNote({{ $return->creditNote->first()->id }})" 
...
function loadInvoiceItems(invoiceId) {
    const url = '{{ route("returns.invoice-items", ":id") }}'.replace(':id', invoiceId);
...
function viewReturn(id) {
    const url = '{{ route("returns.show", ":id") }}'.replace(':id', id);
...
function editReturn(id) {
    const url = '{{ route("returns.edit", ":id") }}'.replace(':id', id);
...
function deleteReturn(id) {
    if (confirm('Are you sure you want to delete this return?')) {
        const url = '{{ route("returns.destroy", ":id") }}'.replace(':id', id);
...
function approveReturn(id) {
    if (confirm('Are you sure you want to approve this return?')) {
        const url = '{{ route("returns.approve", ":id") }}'.replace(':id', id);
...
function confirmReject() {
    ...
    const url = '{{ route("returns.reject", ":id") }}'.replace(':id', currentReturnId);
...
function completeReturn(id) {
    if (confirm('Are you sure you want to complete this return? This will process the refund and restock items.')) {
        const url = '{{ route("returns.complete", ":id") }}'.replace(':id', id);
...
function downloadCreditNote(creditNoteId) {
    const url = '{{ route("returns.credit-note-pdf", ":id") }}'.replace(':id', creditNoteId);
...
function filterReturns() {
    ...
    const url = '{{ route("returns.index") }}' + (params.toString() ? '?' + params.toString() : '');
