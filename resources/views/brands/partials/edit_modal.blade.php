<div class="modal-header bg-warning text-dark">
    <h5 class="modal-title">
        <i class="ri-pencil-line me-2"></i> Edit Brand
    </h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
</div>

<form action="{{ route('brands.update', $brand) }}" method="POST" id="brandEditForm" enctype="multipart/form-data">
    @csrf
    @method('PUT')
        ...
        <div class="row">
            ...
            @if($brand->logo)
                <div class="col-12 mb-3">
                    ...
                    <div>
                        <img src="{{ asset($brand->logo) }}" alt="{{ $brand->name }}" style="max-width: 100px; max-height: 100px; object-fit: cover;">
                    </div>
                </div>
            @endif
            <div class="col-12 mb-3">
                <label class="form-label fw-bold">Description</label>
                <textarea name="description" class="form-control" rows="3" placeholder="Brand description (optional)">{{ $brand->description }}</textarea>
            </div>
        </div>
    </div>
