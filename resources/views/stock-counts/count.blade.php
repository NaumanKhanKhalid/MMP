                    ...
                    <div class="table-responsive" style="max-height: 600px; overflow-y: auto;">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody id="itemsTableBody">
                                ...
                                <tr id="row-{{ $item->id }}" 
                                    class="{{ $item->is_counted ? 'table-success' : '' }}"
                                    data-sku="{{ $item->product->sku ?? '' }}"
                                    data-name="{{ $item->product->name ?? '' }}"
                                    data-barcode="{{ $item->product->barcode_primary ?? '' }}"
                                    data-supplier-code="{{ $item->product->supplier_code ?? '' }}"
                                    data-brand="{{ $item->product->brand->name ?? '' }}"
                                    data-category="{{ $item->product->category->name ?? '' }}"
                                    data-oe-numbers="{{ $item->product->oeNumbers->pluck('oe_number')->implode(',') ?? '' }}">
                                    <td>
                                        @if($item->is_counted)
                                    ...
                                    <td>
                                        <span class="fw-semibold">{{ $item->product->sku }}</span>
                                    </td>
                                    <td>
                                        ...
                                        @if($item->product->barcode_primary)
                                                <small class="badge bg-light text-dark">
                                                    <i class="ri-barcode-line"></i> {{ $item->product->barcode_primary }}
                                                </small>
                                            @endif
                                            @if($item->product->supplier_code)
                                                <small class="badge bg-warning-transparent">
                                                    <i class="ri-building-line"></i> {{ $item->product->supplier_code }}
                                                </small>
                                            @endif
                                        ...
                                        @if($item->product->oeNumbers && $item->product->oeNumbers->count() > 0)
                                            <small class="text-muted d-block mt-1">
                                                <i class="ri-hashtag"></i> OE: {{ $item->product->oeNumbers->take(3)->pluck('oe_number')->implode(', ') }}
                                                @if($item->product->oeNumbers->count() > 3)
                                                    <span class="badge bg-info-transparent">+{{ $item->product->oeNumbers->count() - 3 }} more</span>
                                                @endif
                                            </small>
                                        @endif
                                        @if($item->product->bin_location)
                                            <small class="text-muted">
                                                <i class="ri-map-pin-line"></i> Bin: {{ $item->product->bin_location }}
                                            </small>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="input-group input-group-sm" style="max-width: 200px;">
                                            <button type="button" class="btn btn-outline-secondary" onclick="decrementQty({{ $item->id }})" title="Decrease (-1)">
                                            ...
                                            <button type="button" class="btn btn-outline-secondary" onclick="incrementQty({{ $item->id }})" title="Increase (+1)">
                                                <i class="ri-add-line"></i>
                                            </button>
                                        </div>
                                        <small class="text-muted d-block mt-1">
                                            <i class="ri-information-line"></i> System: <strong class="text-primary">{{ $item->system_qty }}</strong>
                                        </small>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <button type="button" class="btn btn-success save-btn" data-item-id="{{ $item->id }}" onclick="saveItem({{ $item->id }})" title="Save Count">
                                                <i class="ri-save-line me-1"></i> Save
                                            </button>
                                            <button type="button" class="btn btn-outline-info" onclick="setSystemQty({{ $item->id }})" title="Copy System Qty">
...
window.saveItem = function(itemId, silent = false) {
    ...
    fetch('{{ route("stock-counts.update-item", ["countId" => ":countId", "itemId" => ":itemId"]) }}'.replace(':countId', countId).replace(':itemId', itemId), {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            ...
            setTimeout(() => location.reload(), 500);
                ...
                fetch('{{ route("stock-counts.stats", ":id") }}'.replace(':id', countId))
                    .then(r => r.json())
                    .then(stats => {
...
window.startCounting = function() {
    fetch('{{ route("stock-counts.start", ":id") }}'.replace(':id', countId), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
window.completeCounting = function() {
    fetch('{{ route("stock-counts.complete", ":id") }}'.replace(':id', countId), {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
