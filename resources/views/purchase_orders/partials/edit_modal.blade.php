...
<div class="modal-header bg-warning text-dark">
    <h5 class="modal-title">
        <i class="ri-edit-line me-2"></i> Edit Purchase Order #{{ $purchaseOrder->po_number }}
        @if($isApproved)
    ...
    <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="supplier_id" id="editSupplier" class="form-select" required {{ $isApproved ? 'disabled' : '' }}>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <input type="date" name="order_date" id="editOrderDate" 
                               class="form-control" value="{{ $purchaseOrder->order_date->format('Y-m-d') }}" required {{ $isApproved ? 'readonly' : '' }}>
                    </div>
                    <div class="col-md-3 mb-3">
                        <label class="form-label">Expected Delivery Date</label>
                        <input type="date" name="expected_delivery_date" id="editExpectedDate" 
                               class="form-control" value="{{ $purchaseOrder->expected_delivery_date?->format('Y-m-d') }}">
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea name="payment_terms" id="editPaymentTerms" 
                                  class="form-control" rows="2" {{ $isApproved ? 'readonly' : '' }}>{{ $purchaseOrder->payment_terms }}</textarea>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Notes</label>
                        <textarea name="notes" id="editNotes" class="form-control" rows="2">{{ $purchaseOrder->notes }}</textarea>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label class="form-label">Status <span class="text-danger">*</span></label>
                        <select name="status" id="editStatus" class="form-select" required {{ $isApproved ? 'disabled' : '' }}>
        ...
        <div class="card mb-3">
            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-primary" id="editItemCount">{{ $purchaseOrder->items->count() }} items</span>
                </div>
                        ...
                        <tbody id="editItemsBody">
                            ...
                            <tr data-product-id="{{ $item->product_id }}">
                                ...
                                <td class="align-middle">
                                    ...
                                    <input type="hidden" class="edit-item-product-id" value="{{ $item->product_id }}">
                                </td>
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-sm edit-item-qty" 
                                           value="{{ $item->quantity }}" min="1" step="1" {{ $isApproved ? 'readonly' : '' }}>
                                </td>
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-sm edit-item-price" 
                                           value="{{ $item->unit_price }}" min="0" step="0.01" {{ $isApproved ? 'readonly' : '' }}>
                                </td>
                    ...
                    <div class="col-md-8">
                        <div class="form-check mb-2">
                            <input type="checkbox" class="form-check-input" id="editVatEnabled" name="vat_enabled"
                                   {{ $purchaseOrder->vat_enabled ? 'checked' : '' }} {{ $isApproved ? 'disabled' : '' }}>
                            <label class="form-check-label" for="editVatEnabled">
                                Include VAT ({{ $vatSettings['vat_rate'] ?? 15 }}%)
                            </label>
                </div>
            </div>
                    <div class="col-md-4">
                        <table class="table table-sm mb-0">
                            <tr>
                                ...
                                <td class="text-end" id="editSubtotalDisplay">R {{ number_format($purchaseOrder->subtotal, 2) }}</td>
                            </tr>
                            <tr>
                                <td class="text-end">
                                ...
                                <td class="text-end">
                                    <input type="number" name="total_discount" id="editDiscount" 
                                           ...
                                           value="{{ $purchaseOrder->total_discount }}" min="0" step="0.01" {{ $isApproved ? 'readonly' : '' }}>
                                </td>
                            </tr>
                            <tr>
                                <td class="text-end">
                                ...
                                <td class="text-end">
                                    <input type="number" name="shipping" id="editShipping" 
                                           class="form-control form-control-sm text-end" 
                                           value="{{ $purchaseOrder->shipping }}" min="0" step="0.01" {{ $isApproved ? 'readonly' : '' }}>
                                </td>
                            </tr>
                            <tr id="editVatRow" style="display: {{ $purchaseOrder->vat_enabled ? 'table-row' : 'none' }};">
                                <td class="text-end fw-semibold">VAT:</td>
                                <td class="text-end" id="editVatDisplay">R {{ number_format($purchaseOrder->vat, 2) }}</td>
                            </tr>
                            <tr class="table-primary">
                                <td class="text-end fw-bold">Grand Total:</td>
                                <td class="text-end fw-bold fs-18" id="editGrandTotalDisplay">R {{ number_format($purchaseOrder->grand_total, 2) }}</td>
                            </tr>
                        </table>
                </div>
            </div>
        ...
        <!-- Hidden inputs -->
        <input type="hidden" name="subtotal" id="editSubtotal" value="{{ $purchaseOrder->subtotal }}">
        <input type="hidden" name="vat" id="editVat" value="{{ $purchaseOrder->vat }}">
        <input type="hidden" name="grand_total" id="editGrandTotal" value="{{ $purchaseOrder->grand_total }}">
    </div>
...
<script>
$(function() {
    let editItemCounter = {{ $purchaseOrder->items->count() }};
    const vatRate = {{ $vatSettings['vat_rate'] ?? 15 }} / 100;
    ...
    $('#poEditForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '{{ route("purchase-orders.update", $purchaseOrder->id) }}',
            method: 'POST',
            data: formData,
            headers: {
