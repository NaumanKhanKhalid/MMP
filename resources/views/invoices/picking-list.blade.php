...
<body>
    ...
    <div class="header">
            ...
            <div class="contact-info">
                @if(\App\Models\Setting::companyPhone())
                    Phone: {{ \App\Models\Setting::companyPhone() }}
                @endif
                @if(\App\Models\Setting::companyEmail())
                     | Email: {{ \App\Models\Setting::companyEmail() }}
                @endif
            </div>
    </div>
        ...
        <div class="details-grid">
            ...
            <div class="detail-section">
                ...
                <div class="detail-row">Invoice #: {{ $invoice->invoice_number }}</div>
                <div class="detail-row">Date: {{ $invoice->created_at->format('d/m/Y') }}</div>
                <div class="detail-row">Time: {{ $invoice->created_at->format('H:i A') }}</div>
        </div>
    ...
    <table>
            ...
            <tr>
                    ...
                    <td>
                        ...
                        @if($item->product && $item->product->brand)
                            <div class="product-brand">Brand: {{ $item->product->brand->name }}</div>
                    @endif
                </td>
                <td>{{ $item->product_sku }}</td>
                    <td class="barcode-cell">
                        ...
                        @endphp
                        <svg class="barcode-svg barcode" data-barcode="{{ $barcodeValue }}"></svg>
                        <div style="font-size: 9px; margin-top: 2px;">{{ $barcodeValue }}</div>
                    </td>
                    <td style="text-align: center; font-weight: bold;">{{ number_format($item->quantity, 3) }}</td>
                    <td style="text-align: center; font-weight: bold;">{{ $item->product->bin_location ?? 'N/A' }}</td>
        ...
        <div class="footer">
            <div class="footer-line">
                <strong>Total Items to Pick:</strong> {{ $invoice->items->sum('quantity') }} items
            </div>
            <div class="footer-line">
                <strong>Generated on:</strong> {{ now()->format('d/m/Y H:i A') }}
            </div>
            <div class="footer-line">
                <strong>Picking List ID:</strong> PL-{{ $invoice->id }}-{{ $invoice->created_at->format('Ymd') }}
            </div>
        </div>
    </div>
