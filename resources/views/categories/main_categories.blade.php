...
@section('content')
    <div class="container-fluid py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <button class="btn btn-success" id="openCreateCategoryModal">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="avatar avatar-sm p-1 bg-light avatar-rounded">
                                        @if ($category->logo)
                                            <img src="{{ asset($category->logo) }}" alt="{{ $category->name }}"
                                ...
                                <td class="text-end">
                                    <div class="btn-list">
                                        <!-- View -->
                                        <button class="btn btn-sm btn-primary-light btn-icon openViewCategoryModal"
                                            data-id="{{ $category->id }}" title="View">
                                            <i class="ri-eye-line"></i>
                                        </button>

                                        <!-- Edit -->
                                        <button class="btn btn-sm btn-success-light btn-icon openEditCategoryModal"
                                            data-id="{{ $category->id }}" title="Edit">
                                            <i class="ri-pencil-line"></i>
                                        </button>

                                        <!-- Status Toggle -->
                                        <form method="POST" action="{{ route('toggle.category.status', $category->id) }}"
                                            class="d-inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                ...
                                                <i
                                                    class="ri-toggle-{{ $category->status === 'active' ? 'line' : 'fill' }}"></i>
                                            </button>
                                        </form>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                @if ($category->children->count() > 0)
                                                    <div class="alert alert-warning">
                                                        ...
                                                        {{ $category->children->count() }} sub-category(ies).
                                                    </div>
                                                @endif
                                            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center text-muted py-4">
                                    <i class="ri-folder-line fs-48 mb-2"></i>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#openCreateCategoryModal').on('click', function() {
                $.get("{{ route('categories.create-category-modal') }}", function(html) {
