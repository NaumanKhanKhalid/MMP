...
<head>
    ...
    <!-- Node Waves Css -->
    <link href="{{ asset('public/assets/libs/node-waves/waves.min.css') }}" rel="stylesheet">

    <!-- Simplebar Css -->
    <link href="{{ asset('public/assets/libs/simplebar/simplebar.min.css') }}" rel="stylesheet">
...
<body class="pos-container">
            ...
            <div class="col-lg-8">
                ...
                <div class="pos-card p-4 mb-4">
                    <div class="row">
                        ...
                        {{-- <div class="col-md-4">
                            <div class="barcode-scanner" onclick="openBarcodeScanner()">
                                <div class="text-center">
                                    <i class="ri-barcode-box-line fs-1 text-primary mb-2"></i>
                                    <p class="mb-0 fw-semibold">Scan Barcode</p>
                                    <small class="text-muted">Click to scan</small>
                                </div>
                            </div>
                        </div> --}}
                    </div>
    ...
    <script>
        ...
        function createProductRow(product) {
            ...
            row.innerHTML = `
                <div class="avatar avatar-md bg-light me-3">
                    <img src="${product.image || '{{ asset('public/assets/images/products/default.jpg') }}'}" alt="${product.name}" class="rounded" style="width: 50px; height: 50px; object-fit: cover;">
                </div>
        ...
        function searchProductByBarcode(barcode) {
            fetch(`{{ route('pos.product-by-barcode') }}?barcode=${encodeURIComponent(barcode)}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
        ...
        function generatePDF() {
            if (currentInvoiceId) {
                window.open(`{{ url('pos/invoice-pdf') }}/${currentInvoiceId}`, '_blank');
            } else {
        ...
        function printInvoice() {
            if (currentInvoiceId) {
                const printWindow = window.open(`{{ url('pos/invoice-pdf') }}/${currentInvoiceId}`, '_blank');
        ...
        function generatePickingList() {
            if (currentInvoiceId) {
                window.open(`{{ url('pos/picking-list') }}/${currentInvoiceId}`, '_blank');
            } else {
        ...
        function sendWhatsApp() {
            ...
            fetch(`{{ url('pos/send-invoice') }}/${currentInvoiceId}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function saveAsQuote() {
            ...
            showWarningModal(
                ...
                function() {
                    window.location.href = '{{ route('quotes.create') }}?cart=' + encodeURIComponent(JSON.stringify(cart));
                    const modal = bootstrap.Modal.getInstance(document.getElementById('warningModal'));
                    modal.hide();
                }
            );
